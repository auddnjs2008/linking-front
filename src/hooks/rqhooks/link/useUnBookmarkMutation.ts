import { unBookmarkLink } from "@/service/link/unbookmarkLink";
            ...
            const newData: PaginationData = {
              ...
              pages: paginationData.pages.map((page) => ({
                ...
                data: page.data.map((link) =>
                  link.id === id ? { ...link, isBookmarked: false } : link
                ),
              })),
            };
        ...
        .forEach(([queryKey, data]) => {
          ...
          const newData = {
            ...
            linkedLinks: groupData.linkedLinks.map((link) =>
              link.id !== id ? link : { ...link, isBookmarked: false }
            ),
          };
